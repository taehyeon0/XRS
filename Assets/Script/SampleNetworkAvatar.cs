...
public class SampleNetworkAvatar : OvrAvatarEntity
{
...
#pragma warning restore 618 // restore deprecated method call warnings
    ...
    void RecordAndSendStreamDataIfMine()
    {
        if (m_photonView.IsMine)
        {
            //Debug.LogError("THAN: Send Avatar data.");
    ...
    {
        //Debug.LogError("THAN: Recieve Avatar data.");
    ...
    void Update()
    {
        ...
        {
           // Debug.LogError("THAN: m_streamedDataList is not NULL");
            if (IsLocal == false)
            {
                //Debug.LogError("THAN: IsLocal is false");
                byte[] firstBytesInList = m_streamedDataList[0];
                if (firstBytesInList != null)
                {
                    //Debug.LogError("THAN: firstBytesInList Not NULL");
